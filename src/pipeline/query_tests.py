...
# query to return all tags and their counts
print("\ntag breakdown in markets:")
print(con.execute("""
    SELECT tag, COUNT(*) AS markets 
    FROM (SELECT UNNEST(tags) as tag FROM markets)
    GROUP BY tag
    ORDER BY markets DESC
    LIMIT 30
""").fetchdf().to_string(index=False))
...
print("\n data range for markets we have trades for ")
print(con.execute("""
    SELECT 
    MIN(m.closedTime) as earliest_close,
    MAX(m.closedTime) as latest_close,
    COUNT(DISTINCT m.conditionId) as markets_with_trades
FROM markets m
INNER JOIN trades t ON t.condition_id = m.conditionId
